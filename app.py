...
# ─────────────────────────────────────────────────────────────
# TEMA: AZUL CLARO · PRETO · BRANCO · TRANSPARÊNCIAS
# ─────────────────────────────────────────────────────────────
st.markdown("""
<style>
...
.stApp::before {
    ...
    background:
        radial-gradient(ellipse at 20% 20%, rgba(147,210,255,0.06) 0%, transparent 60%),
...
[data-testid="stSidebar"] {
    ...
    border-right: 1px solid rgba(147,210,255,0.15);
}
[data-testid="stSidebar"] * {
...
[data-testid="stDataFrame"] {
    border: 1px solid rgba(147,210,255,0.15) !important;
...
/* ── Divisor ── */
hr {
    border-color: rgba(147,210,255,0.12) !important;
...
/* ── Toggle ── */
[data-testid="stToggle"] { accent-color: #3b82f6; }
...
[data-testid="stSelectbox"] label { color: rgba(255,255,255,0.8) !important; }
</style>
...
CMAP_BLUE = mcolors.LinearSegmentedColormap.from_list(
    "blue_grad", ["#0a1628", "#1d4ed8", "#3b82f6", "#93d2ff", "#e0f2ff"]
)
...
def tema_grafico():
    plt.rcParams.update({
        ...
        "axes.labelcolor":   TEXTO_G,
        "xtick.color":       TEXTO_G,
        "ytick.color":       TEXTO_G,
        "text.color":        TEXTO_G,
        "grid.color":        "#0d2040",
        "grid.linewidth":    0.6,
        "legend.facecolor":  "#050d1a",
...
st.markdown("""
<div style="
    background: linear-gradient(135deg, rgba(10,22,40,0.95) 0%, rgba(29,78,216,0.3) 100%);
...
">
    <div style="display:flex; align-items:center; gap:14px;">
        ...
        <div>
            <h1 style="margin:0; font-size:1.75rem; color:white !important;
                       letter-spacing:-0.02em;">
                Asset Allocation <span style="color:#93d2ff;">Otimizado</span>
            </h1>
            <p style="margin:4px 0 0 0; color:rgba(200,230,255,0.6) !important;
                      font-size:0.88rem;">
                Modelo quantitativo · Markowitz · Black-Litterman · Mercado Brasileiro
            </p>
        </div>
    </div>
</div>
...
if rodar:
    ...
    # ── Black-Litterman (session_state fix) ──────────────────
    views = {}
    if usar_bl:
        st.markdown("---")
        ...
        for i, ativo in enumerate(ativos):
            ...
            with cols[i]:
                ...
                if tp > 0:
                    ...
                    views[ativo] = upside
                    cor = "#60efb0" if upside > 0 else "#f87171"
                    st.markdown(
                        f'<p style="color:{cor};font-weight:600;">'
                        f"Upside: {upside:.1%}</p>",
                        unsafe_allow_html=True
                    )
                else:
                    st.caption("usa prior de mercado")

        confianca = st.slider(
            "Confiança nas views", 0.0, 1.0, 0.5, key="bl_confianca"
        )
    ...
    if min_w * n > 1.0:
        min_w = 0.0
        st.warning("Peso mínimo ajustado para 0% — restrição inviável.")
    ...
    with tab1:
        ...
        with col1:
            ...
            n_al      = len(alocacao)
            cores_pie = [CMAP_BLUE(i / max(n_al - 1, 1)) for i in range(n_al)]

            fig_pie, ax = plt.subplots(figsize=(5, 5), facecolor=PRETO)
            ax.set_facecolor(PRETO)
            wedges, texts, autotexts = ax.pie(
                alocacao.values(),
                labels=alocacao.keys(),
                autopct="%1.1f%%",
                colors=cores_pie,
                wedgeprops=dict(edgecolor=PRETO, linewidth=2.5,
                                width=0.65),
                startangle=90,
                pctdistance=0.75,
            )
            ...
            ax.set_title("Composição do Portfólio",
                         color=AZUL_BRILHO, fontweight="bold", pad=16)
    ...
    with tab2:
        ...
        with col1:
            ...
            ax.set_facecolor(PRETO)
            sns.heatmap(
                corr, annot=True, cmap=CMAP_BLUE,
                fmt=".2f", vmin=-1, vmax=1,
                ax=ax, linewidths=0.5, linecolor="#050d1a",
                annot_kws={"color": BRANCO, "fontsize": 9}
            )
            ax.set_title("Correlação", color=AZUL_BRILHO, fontweight="bold")
            ax.tick_params(colors=TEXTO_G)
        ...
        with col2:
            ...
            sns.heatmap(
                corr_ord, annot=True, cmap=CMAP_BLUE,
                fmt=".2f", vmin=-1, vmax=1,
                ax=ax2, linewidths=0.5, linecolor="#050d1a",
                annot_kws={"color": BRANCO, "fontsize": 9}
            )
            ax2.set_title("Agrupada por Hierarquia",
                          color=AZUL_BRILHO, fontweight="bold")
            ax2.tick_params(colors=TEXTO_G)
    ...
    with tab3:
        ...
        fig_ef, ax = plt.subplots(figsize=(11, 6), facecolor=PRETO)
        ax.set_facecolor("#050d1a")

        sc = ax.scatter(
            res[1] * 100, res[0] * 100,
            c=res[2], cmap=CMAP_BLUE,
            alpha=0.35, s=7
        )
        cb = plt.colorbar(sc, ax=ax)
        cb.set_label("Sharpe Ratio", color=TEXTO_G)
        cb.ax.yaxis.set_tick_params(color=TEXTO_G)
        plt.setp(cb.ax.yaxis.get_ticklabels(), color=TEXTO_G)

        mkrs = {
            "Max Sharpe":       ("*", AZUL_BRILHO, 380),
            "Min Volatilidade": ("D", "#bfdbfe",   150),
            "Equal Weight":     ("s", BRANCO,      150),
            "Risk Parity":      ("^", "#fbbf24",   150),
        }
        for nome, p in portfolios.items():
            m, c, sz = mkrs[nome]
            ax.scatter(p["vol"] * 100, p["ret"] * 100,
                       marker=m, color=c, s=sz,
                       edgecolors=PRETO, linewidth=0.8,
                       label=nome, zorder=5)

        ax.set_xlabel("Volatilidade (%)", color=TEXTO_G)
        ax.set_ylabel("Retorno Esperado (%)", color=TEXTO_G)
        ax.set_title("Fronteira Eficiente + Monte Carlo",
                     color=AZUL_BRILHO, fontweight="bold")
        ax.legend(facecolor="#050d1a", edgecolor="#1e3a5f",
                  labelcolor=TEXTO_G)
        ax.grid(alpha=0.15, color="#1e3a5f")
    ...
    with tab4:
        ...
        ax0 = axes[0]
        ax0.plot((port_rebal - 1) * 100,
                 color=AZUL_BRILHO, linewidth=2,
                 label="Portfólio (rebalanceado)")
        ax0.plot((port_static - 1) * 100,
                 color=AZUL_CLARO, linewidth=1.5, linestyle="--",
                 label="Portfólio (buy-and-hold)")
        ax0.plot((ibov_cum.reindex(port_rebal.index).ffill() - 1) * 100,
                 color="#94a3b8", alpha=0.7, label="IBOV")
        ax0.plot((bova11_cum.reindex(port_rebal.index).ffill() - 1) * 100,
                 color="#cbd5e1", alpha=0.7, label="BOVA11")
        ax0.plot((cdi_cum - 1) * 100,
                 color="#fbbf24", linewidth=1.5, linestyle=":",
                 label="CDI")
        ax0.set_title("Performance Comparada",
                      color=AZUL_BRILHO, fontweight="bold")
        ax0.set_ylabel("Retorno Acumulado (%)", color=TEXTO_G)
        ax0.legend(facecolor="#050d1a", edgecolor="#1e3a5f",
                   labelcolor=TEXTO_G, fontsize=9)
        ax0.grid(alpha=0.15, color="#1e3a5f")

        ax1 = axes[1]
        ret_anuais = port_rebal.resample("YE").last().pct_change().dropna() * 100
        cores_bar  = [AZUL_BRILHO if v >= 0 else "#f87171"
                      for v in ret_anuais.values]
        ax1.bar(ret_anuais.index.year, ret_anuais.values,
                color=cores_bar, edgecolor=PRETO, alpha=0.85)
        ax1.axhline(0, color=TEXTO_G, linewidth=0.8)
        ax1.set_title("Retorno Anual (%)",
                      color=AZUL_BRILHO, fontweight="bold")
        ax1.set_ylabel("Retorno (%)", color=TEXTO_G)
        ax1.grid(axis="y", alpha=0.15, color="#1e3a5f")
    ...
    with tab5:
        ...
        axes[0].fill_between(drawdown.index, drawdown * 100, 0,
                             color="#f87171", alpha=0.4)
        axes[0].plot(drawdown * 100, color="#ef4444", linewidth=0.8)
        axes[0].set_title("Drawdown (%)", color=AZUL_BRILHO, fontweight="bold")
        axes[0].grid(alpha=0.15, color="#1e3a5f")

        axes[1].hist(port_ret_d * 100, bins=60,
                     color=AZUL_MED, alpha=0.75, edgecolor=PRETO)
        axes[1].axvline(var_hist * 100, color="#fbbf24",
                        linewidth=2, linestyle="--",
                        label=f"VaR 95%: {var_hist:.2%}")
        axes[1].axvline(cvar_hist * 100, color="#f87171",
                        linewidth=2, linestyle="--",
                        label=f"CVaR: {cvar_hist:.2%}")
        axes[1].set_title("Distribuição de Retornos Diários",
                          color=AZUL_BRILHO, fontweight="bold")
        axes[1].legend(facecolor="#050d1a", edgecolor="#1e3a5f",
                       labelcolor=TEXTO_G)
        axes[1].grid(alpha=0.15, color="#1e3a5f")
        ...
        if rows:
            ...
            fig_st, ax = plt.subplots(
                figsize=(12, max(4, len(nomes) * 0.85)),
                facecolor=PRETO
            )
            ax.set_facecolor("#050d1a")
            ...
            ax.barh(x + width / 2, ret_ps, width,
                    label="Portfólio",
                    color=[AZUL_BRILHO if v >= 0 else "#f87171" for v in ret_ps],
                    alpha=0.9)
            ax.barh(x - width / 2, ret_bs, width,
                    label="BOVA11",
                    color=["#bfdbfe" if v >= 0 else "#fca5a5" for v in ret_bs],
                    alpha=0.6)

            ax.set_yticks(x)
            ax.set_yticklabels(nomes, color=TEXTO_G, fontsize=9)
            ax.axvline(0, color=TEXTO_G, linewidth=0.8)
            ax.set_xlabel("Retorno (%)", color=TEXTO_G)
            ax.set_title("Stress Testing — Portfólio vs BOVA11",
                         color=AZUL_BRILHO, fontweight="bold")
            ax.legend(facecolor="#050d1a", edgecolor="#1e3a5f",
                      labelcolor=TEXTO_G)
            ax.grid(axis="x", alpha=0.15, color="#1e3a5f")

            plt.tight_layout(pad=2.5)
            st.pyplot(fig_st, use_container_width=True)
        else:
            st.info("Sem dados suficientes para os cenários de stress. "
                    "Tente ampliar o período de análise (mínimo 2020).")

    # ── ABA 6: Walk-Forward ──────────────────────────────────
    with tab6:
        st.info(
            "Otimiza em janelas de 2 anos e testa nos 3 meses seguintes "
            "— elimina look-ahead bias e simula uso real do modelo."
        )
        ...
        with st.spinner("🔁 Rodando Walk-Forward — pode levar alguns minutos..."):
        ...
        if wf_returns:
            ...
            ax.plot(wf_cum, color=AZUL_BRILHO, linewidth=2,
                    label="Walk-Forward (realista)")
            ax.plot(static_p, color="#94a3b8",
                    linewidth=1.5, linestyle="--",
                    label="Buy-and-Hold (mesmo período)")
            ax.fill_between(wf_cum.index, wf_cum, static_p,
                            where=(wf_cum >= static_p),
                            alpha=0.08, color=AZUL_BRILHO)
            ax.fill_between(wf_cum.index, wf_cum, static_p,
                            where=(wf_cum < static_p),
            ...
            ax.set_title("Walk-Forward vs Buy-and-Hold",
                         color=AZUL_BRILHO, fontweight="bold")
            ax.set_ylabel("Retorno Acumulado", color=TEXTO_G)
            ax.legend(facecolor="#050d1a", edgecolor="#1e3a5f",
                      labelcolor=TEXTO_G)
            ax.grid(alpha=0.15, color="#1e3a5f")
            ...
            st.warning(
                "Dados insuficientes para o Walk-Forward. "
